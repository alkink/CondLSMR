...
def argument_parser():
    ...
    parser.add_argument('--img-size', nargs=2, type=int, default=None, metavar=('W', 'H'),
                        help='Eğitimle aynı olmalı. OOM için 800 320. Verilmezse dataset varsayılanı.')
    ...
    parser.add_argument('--model-path', type=str, metavar='PATH', default=None, help="pretrained backbone path (for model creation)")
    parser.add_argument('--checkpoint', type=str, metavar='PATH', default=None, help="path to checkpoint .pth.tar file (overrides --logs-dir)")
    parser.add_argument('--score-thresh', type=float, default=None,
                        help="override detector score threshold during inference")
    # distributed
...
def main(args):
    ...
    model = models.create(args.arch, norm_layer=norm_layer, num_classes=args.num_classes,
                          model_path=args.model_path, enc_src_hw=args.enc_src_hw,
                          score_thresh=args.score_thresh if args.score_thresh is not None else 0.7).eval()
    if not args.distributed or args.local_rank == 0:
        n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print('number of params:', n_parameters)

    # Load from checkpoint: --checkpoint (file) > logs-dir/model_best > logs-dir/checkpoint
    if args.checkpoint and os.path.isfile(args.checkpoint):
        checkpoint_path = args.checkpoint
    else:
        checkpoint_path = os.path.join(args.logs_dir, 'model_best.pth.tar')
        if not os.path.exists(checkpoint_path):
            checkpoint_path = os.path.join(args.logs_dir, 'checkpoint.pth.tar')
