...
def load_culane_data(data_dir, file_list_path):
    if refile is not None:
        file_obj = refile.smart_open(file_list_path, 'r')
    else:
        file_obj = open(file_list_path, 'r')
    with file_obj as file_list:
        filepaths = [
            os.path.join(
...
def main():
    ...
    if args.translate_v1:
        pred_dict = {}
        open_fn = refile.smart_open if refile is not None else open
        with open_fn(os.path.join(args.pred_dir, 'results.pkl'), 'rb') as f:
            results_list = pickle.load(f)
        for result in tqdm(results_list):
            pred_dict[result['image_name'].replace('.jpg', '').replace('_', ',')] = result['2d_res']

        if refile is None:
            raise RuntimeError('translate_v1 requires refile for remote s3 assets')
        ...
        for result in tqdm(results_list):
            ...
            if image_name.startswith('/'):
                image_rel = image_name[1:]
            else:
                image_rel = image_name.replace(',', '/')
            res_path = os.path.join(args.pred_dir, image_rel.replace('.jpg', '.lines.txt'))
            ann_path = os.path.join(args.anno_dir, image_rel.replace('.jpg', '.lines.txt'))
            if not os.path.exists(ann_path):
                print(ann_path)
                continue

            res_list = result.get('2d_res', result.get('points'))
            if res_list is None:
                raise KeyError("Expected '2d_res' or 'points' in test results")
            os.makedirs(os.path.dirname(res_path), exist_ok=True)
            with open(res_path, 'w') as f:
                for lane in res_list:
                    lane = _lane_to_flat_values(lane)
                    f.write(' '.join(list(map(str, lane))) + '\n')
