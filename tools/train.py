...
def argument_parser():
    ...
    parser.add_argument('--train-split', type=str, default='train', help='Train split (örn. train, train_2k)')
    parser.add_argument('--val-split', type=str, default='val', help='Val/test split (örn. val, test_2k)')
    parser.add_argument('-v', '--version', type=str, default='v1.0')
    parser.add_argument('--img-size', nargs=2, type=int, default=None, metavar=('W', 'H'),
                        help='Görüntü boyutu (genişlik yükseklik). OOM için 800 320 önerilir. Verilmezse dataset varsayılanı (culane: 1600x640) kullanılır.')
    parser.add_argument('--enc-src-hw', nargs=2, type=int, default=None, metavar=('H', 'W'),
                        help='Transformer encoder source grid override. Örn: 24 42')
    ...
    parser.add_argument('--max-steps', type=int, default=0,
                        help="maximum global train steps; 0 disables early stop")
    # misc
...
def main(args):
    ...
    if getattr(args, 'img_size', None) and len(args.img_size) == 2:
        transform_kw['img_w'], transform_kw['img_h'] = args.img_size[0], args.img_size[1]
    train_transforms, train_collate_fn = transforms.create(
        args.dataset, train=True, **transform_kw)
    test_transforms, test_collate_fn = transforms.create(
        args.dataset, train=False, **transform_kw)

    if args.test_dataset is None:
        args.test_dataset = args.dataset

    train_dataset = datasets.create(
        args.dataset, data_root, split=args.train_split, transform=train_transforms, version=args.version)
    test_dataset = datasets.create(
        args.test_dataset, data_root, split=args.val_split, transform=test_transforms, version=args.version)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        should_stop = trainer(train_loader, epoch, best_prec1)
        ...
        if should_stop:
            print("Reached max_steps={}, stopping training early.".format(args.max_steps))
            break
