...
## Pre-trained Checkpoints (Önceden Eğitilmiş Modeller)

**Resmi checkpoint indirme linki yok.** Orijinal repo ([czyczyyzc/CondLSTR](https://github.com/czyczyyzc/CondLSTR)) ve paper'da önceden eğitilmiş SOTA checkpoint paylaşılmamış. Seçenekler:

- **Kendi eğitiminiz:** Yukarıdaki Training komutlarıyla modeli eğitin; checkpoint'ler `--logs-dir` içinde `checkpoint.pth.tar` ve `model_best.pth.tar` olarak kaydedilir.
- **Checkpoint nerede aranır:** Test/eval için script `--logs-dir` altında sırayla `model_best.pth.tar`, yoksa `checkpoint.pth.tar` dosyasına bakar. İsterseniz doğrudan dosya yolu için: `tools/test.py ... --checkpoint /path/to/checkpoint.pth.tar`
- **Yazarlarla iletişim:** Pre-trained model isteği için [GitHub Issues](https://github.com/czyczyyzc/CondLSTR/issues) kullanılabilir.
