...
def _batch_norm_1x1_safe(x, bn_module):
    """(1, C, 1, 1) için BN: training=False ile running stats kullan (mod değiştirmeden)."""
    return F.batch_norm(
        x,
...
class ConvBNReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.dim() == 4 and x.size(0) == 1 and x.size(2) == 1 and x.size(3) == 1 and self.bn.training:
            x = _batch_norm_1x1_safe(x, self.bn)
        else:
            x = self.bn(x)
        x = self.relu(x)
        return x
...
class AttentionRefinementModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if atten.dim() == 4 and atten.size(0) == 1 and atten.size(2) == 1 and atten.size(3) == 1 and self.bn_atten.training:
            atten = _batch_norm_1x1_safe(atten, self.bn_atten)
        else:
