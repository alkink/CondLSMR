...
class Transformer(nn.Module):
    ...
    def __init__(self, in_channels, src_shape=None, tgt_shape=(32, 32), d_model=512, n_heads=8,
                 ...
                 src_down_scale=None, tgt_down_scale=None, mem_down_scale=None, enc_src_hw=None):
        super().__init__()
        if use_input_proj:
        ...
        if enc_src_hw is None:
            self.enc_src_hw = None
        else:
            if not isinstance(enc_src_hw, (tuple, list)) or len(enc_src_hw) != 2:
                raise ValueError('enc_src_hw must be (H, W) when provided')
            self.enc_src_hw = (int(enc_src_hw[0]), int(enc_src_hw[1]))
            if self.enc_src_hw[0] <= 0 or self.enc_src_hw[1] <= 0:
                raise ValueError('enc_src_hw values must be positive')
    ...
    def forward(self, src, src_mask=None, attn_mask=None):
        ...
        decoder_output = self.forward_decoder(encoder_output, attn_mask=attn_mask)
    ...
    def forward_encoder(self, src, src_mask=None, src_pos_embed=None):
        ...
        if self.enc_src_hw is not None:
            target_hw = tuple(self.enc_src_hw)
            if len(src.shape) == 4:
                if tuple(src.shape[-2:]) != target_hw:
                    src = F.interpolate(src, size=target_hw, mode='bilinear', align_corners=True)
                if src_mask is not None and tuple(src_mask.shape[-2:]) != target_hw:
                    src_mask = F.interpolate(src_mask[:, None].float(), size=target_hw, mode='nearest')[:, 0].bool()
            else:
                if tuple(src.shape[-2:]) != target_hw:
                    bs, d_src, c_src, _, _ = src.shape
                    src = src.flatten(0, 1)
                    src = F.interpolate(src, size=target_hw, mode='bilinear', align_corners=True)
                    src = src.view(bs, d_src, c_src, *target_hw)
                if src_mask is not None and tuple(src_mask.shape[-2:]) != target_hw:
                    bs, d_src, _, _ = src_mask.shape
                    src_mask = src_mask.flatten(0, 1)[:, None].float()
                    src_mask = F.interpolate(src_mask, size=target_hw, mode='nearest')[:, 0].bool()
                    src_mask = src_mask.view(bs, d_src, *target_hw)
        ...
        memory = self.encoder(src,
                              ...
                              src_shape=src_shape)                                                             # (H * W, N, C)
        return memory, src_mask, src_pos_embed, src_shape

    def forward_decoder(self, encoder_output, tgt=None, tgt_mask=None, tgt_pos_embed=None, attn_mask=None):
        memory, src_mask, src_pos_embed, src_shape = encoder_output
        bs, c = memory.shape[-2:]
        d_src, h_src, w_src = src_shape
        ...
        d_tgt = None
        tgt_shape = (d_tgt, h_tgt, w_tgt)
            
        if self.mem_down_scale is not None:
            memory = memory.view(h_src, w_src, bs, -1).permute(2, 3, 0, 1).contiguous()                        # (N, C, H, W)
            ...
            src_pos_embed = src_pos_embed.view(h_src, w_src, bs, -1).permute(2, 3, 0, 1).contiguous()          # (N, C, H, W)
            ...
            if src_mask is not None:
                src_mask = src_mask.view(bs, h_src, w_src)[:, None].float()                                    # (N, 1, H, W)
        ...
        if attn_mask is not None:
            if len(attn_mask.shape) == 4:
                attn_mask = attn_mask.unsqueeze(dim=0).repeat(bs, 1, 1, 1, 1)                                  # (N, H, W, H, W)
            attn_mask = attn_mask.flatten(1, 2).flatten(2, 3)                                                  # (N, H * W, H * W)
            attn_mask = attn_mask.repeat_interleave(self.n_heads, dim=0)                                       # (N * h, H * W, H * W)
        ...
        if self.use_fix_encode:
            ...
            if tgt_mask is None:
                tgt_mask = torch.zeros((bs, h_tgt, w_tgt), dtype=torch.bool, device=memory.device)             # (N, H, W)
            if tgt_pos_embed is None:
        ...
        hs = self.decoder(tgt, 
                          ...
                          memory_pos_embed=src_pos_embed,
                          tgt_shape=tgt_shape)                                                                 # (M, H * W, N, C)
        ys = [x.view(h_tgt, w_tgt, bs, -1).permute(2, 3, 0, 1).contiguous() for x in hs]
        return ys, hs, tgt_mask, tgt_pos_embed
...
# S = (X.mm(X.t()) + 1.0) / 2.0 + 1e-5
# S = S.cpu().numpy()
# M = np.ones_like(S)
# M = np.triu(M, k=1).astype(np.bool)
# S = S[M]
#
# plt.figure(figsize=(6, 4))  ## 设置画布\n",
# plt.hist(S, bins=50)
# plt.xlim(0, 1)
# plt.savefig('/data/gpfs/projects/punim1962/project/count.jpg')
#
# import sys
# sys.exit()
