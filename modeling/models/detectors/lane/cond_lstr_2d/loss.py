...
class CondLSTR2DLoss(nn.Module):
    ...
    def preprocess(self, gt_masks, gt_labels, mask_h, mask_w, img_metas):
        ...
        for gt_mask, gt_attr, img_meta in zip(gt_masks, gt_labels, img_metas):
            ...
            # Uyumluluk: batch'te img_mask M_max'e padlenir; lane_attris M_i uzunluğunda. Her iki yönde de eşleştir.
            n_attr = len(gt_attr) if gt_attr is not None else len(gt_mask)
            n_lanes = min(len(gt_mask), n_attr)
            gt_mask = gt_mask[:n_lanes]
            if gt_attr is not None and len(gt_attr) > n_lanes:
                gt_attr = gt_attr[:n_lanes]
            if len(gt_mask) > 0:
                if gt_mask.dim() == 2:
                    # gt_mask shape is (M, W) - need to make it (M, 1, 1, W) for interpolate
                    M, W = gt_mask.shape
                    gt_mask = gt_mask.unsqueeze(1).unsqueeze(2).float()  # (M, 1, 1, W)
                else:
            ...
            if gt_attr is None:
                ...
                if not torch.is_tensor(gt_attr):
                    gt_attr = torch.from_numpy(np.asarray(gt_attr, dtype=np.int64)).to(device)
