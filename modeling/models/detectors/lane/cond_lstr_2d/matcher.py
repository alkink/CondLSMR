...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, logits_obj, logits_cls, regs, masks, lane_ranges, targets):
        ...
        for i, tgt in enumerate(targets):
            ...
            # Compute the regression cost in chunks over L to avoid OOM (L, M, H, W)
            out_row_regs = regs[i]                                                                     # (L, H, W)
            tgt_row_regs = tgt['gt_row_reg']                                                           # (M, H, W)
            tgt_row_msks = tgt['gt_row_reg_mask']                                                      # (M, H, W)
            L, M = out_row_regs.size(0), tgt_row_regs.size(0)
            chunk_size = 1
            cost_row_reg_chunks = []
            for start in range(0, L, chunk_size):
                end = min(start + chunk_size, L)
                out_chunk = out_row_regs[start:end].unsqueeze(1).repeat(1, M, 1, 1)                    # (chunk, M, H, W)
                tgt_chunk = tgt_row_regs.unsqueeze(0).repeat(end - start, 1, 1, 1)                     # (chunk, M, H, W)
                tgt_msk_exp = tgt_row_msks.unsqueeze(0).expand(end - start, -1, -1, -1)                # (chunk, M, H, W)
                c = F.l1_loss(out_chunk, tgt_chunk, reduction='none') * tgt_msk_exp
                c = c.sum(dim=(2, 3)) / tgt_row_msks.sum(dim=(1, 2)).clamp(min=1.0).unsqueeze(0).expand(end - start, -1)
                cost_row_reg_chunks.append(c)
            cost_row_reg = torch.cat(cost_row_reg_chunks, dim=0)                                       # (L, M)
