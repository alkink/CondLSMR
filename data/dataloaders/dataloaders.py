...
def simple_dataloader(dataset, batch_size, train=True, distributed=False, num_workers=4, prefetch_factor=2,
    ...
    # num_workers=0 iken prefetch_factor verilmemeli (PyTorch kısıtı)
    loader_kw = dict(dataset=dataset, batch_size=batch_size, sampler=sampler, num_workers=num_workers,
                     pin_memory=True, drop_last=train, collate_fn=collate_fn)
    if num_workers > 0:
        loader_kw['prefetch_factor'] = prefetch_factor
    dataloader = DataLoader(**loader_kw)
...
def parquet_dataloader(dataset, batch_size, train=True, distributed=False, num_workers=4, prefetch_factor=2,
    ...
    loader_kw = dict(dataset=dataset, batch_sampler=batch_sampler, num_workers=num_workers,
