...
# OpenCV + imgaug bazı ortamlarda (özellikle CUDA yoğun yük altında) nadiren segfault atabiliyor.
# Thread/OpenCL'i kapatmak genelde stabiliteyi artırır.
try:
    cv2.setNumThreads(0)
...
class GenerateLaneLine2D(object):
    ...
    def __call__(self, results):
        ...
        if len(lane_points) > 0:
            ...
            # Boş lane: 0 kanallı mask (H, W, 0) – collate ve loss 0 lane kabul ediyor
            img_mask = np.zeros((img.shape[0], img.shape[1], 0), dtype=np.uint8)
        ...
        if 'lane_attris' in results:
            ...
            # CULane gibi lane_attris olmayan veri setleri için varsayılan (0 = tek sınıf)
            results['lane_attris'] = np.zeros(len(lane_points), dtype=np.int64)
